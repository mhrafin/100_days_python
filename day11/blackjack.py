...
\______   \  | _____    ____ |  | __     |    |____    ____ |  | __
 ...
 |______  /____(____  /\___  >__|_ \ \________(____  /\___  >__|_ \
    ...
    if sum(dealers_hand) > 21:
        print(f"Sum of Dealers hand is bigger than 21. {dealers_hand} =  {sum(dealers_hand)}")
        print("You Win!\n")
    ...
    if sum(players_hand) > 21:
        print(f"Sum of Player's hand is bigger than 21. {players_hand} =  {sum(players_hand)}")
        print("You Lost\n")
        play_again = input("Do want to play again?\nType 'y' for Yes or 'n' for No.\n").lower()
        if play_again == "y":
            os.system('cls' if os.name == 'nt' else 'clear')
    ...
    if sum(players_hand) > sum(dealers_hand):
        ...
        print(f"{players_hand} = {sum(players_hand)} > {dealers_hand} = {sum(dealers_hand)}")
        print("You Win!\n")
        play_again = input("Do want to play again?\nType 'y' for Yes or 'n' for No.\n").lower()
        if play_again == "y":
            os.system('cls' if os.name == 'nt' else 'clear')
...
def card(for_player: bool):
    if for_player:    
        ...
        if not card_drawn == 11: 
            ...
            one_or_eleven = int(input("Type 1 to count it as one or\nType 11 to count it as eleven\n"))
            if one_or_eleven == 1:
                return 1
            else:
                return 11
